...
namespace Patchouli {
  ...
  int server::operator()() {
    ...
    managed_shared_memory::size_type _free_memory = segment.get_free_memory();
    void* _shptr = segment.allocate(1024/*bytes to allocate*/);

    segment.construct<sharedString>("sharedString")(server_name.c_str(), segment.get_segment_manager());

    // launch a work thread
    boost::thread _thread(&server::worker, this);
